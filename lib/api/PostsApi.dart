...
class PostsAPI {
  ...
  Future<List<Post>> fetchRecentUpdates() async {
    String recentUpdatesApi = base_api + recent_updates_api;
    var response = await http.get(recentUpdatesApi);
    List<Post> posts = List<Post>();
    if( response.statusCode == 200 ){
      var jsonData = jsonDecode(  response.body);
      var data = jsonData["data"];
  ...
  Future<List<Post>> fetchPopularPosts() async {
    String popularPostsApi = base_api + popular_posts_api;
    var response = await http.get(popularPostsApi);
    List<Post> posts = List<Post>();
    if( response.statusCode == 200 ){
      var jsonData = jsonDecode(  response.body);
      var data = jsonData["data"];
